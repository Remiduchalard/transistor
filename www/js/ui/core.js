...
const UI = {
    ...
    formatExactPrice(val) {
        ...
        let coefStr = numParts[0].replace('.', '').replace(/0+$/, '');
        const exponent = parseInt(numParts[1]); // e.g. -4

        // Construct the string manually to avoid toFixed precision limits (RangeError: precision out of range)
        let str = "0." + "0".repeat(Math.abs(exponent) - 1) + coefStr;
