...
UI.Stats = {
    ...
    update() {
        ...
        UI.els.stock.textContent = UI.formatNumber(Game.state.transistors);
        UI.els.perYear.textContent = I18n.t("per_year", { val: UI.formatNumber(Game.productionPerYear) });
        UI.els.money.textContent = UI.formatMoney(Game.state.money);
        ...
        UI.els.playTime.textContent = UI.formatTime(Game.state.virtualElapsed);
        
        let displayClickPower = Game.clickPower;
        if (Game.state.boostMs > 0) {
        ...
        // Boost UI
        UI.els.boostProgressText.textContent = Game.state.purchaseCounter + " / 100";
        UI.els.boostProgressBar.style.width = Game.state.purchaseCounter + "%";
        
        if (Game.state.boostMs > 0) {
            UI.els.boostActiveDisplay.style.display = "block";
            UI.els.boostTimer.textContent = (Game.state.boostMs / 1000).toFixed(1);
        } else {
            UI.els.boostActiveDisplay.style.display = "none";
        }
        
        if (Game.state.consumables > 0) {
            UI.els.useBoostBtn.style.display = "block";
            UI.els.boostStock.textContent = Game.state.consumables;
        } else {
        ...
        // Year progress
        const yp = getYearProgress(Game.state.totalTransistors);
        UI.els.yearTotalProduced.textContent = UI.formatNumber(Game.state.totalTransistors);
        ...
        if (advancedTotalEl) advancedTotalEl.textContent = UI.formatNumber(Game.state.totalTransistors);
        
        // Exact numbers for Settings tab
        document.querySelectorAll(".exact-total-produced").forEach(el => {
            el.textContent = new Decimal(Game.state.totalTransistors).floor().toNumber().toLocaleString(I18n.lang === 'fr' ? 'fr-FR' : 'en-US');
        });
